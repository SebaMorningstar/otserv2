...
local player_positions = {
    [1] = {fromPos = Position(33638, 32562, 13), toPos = Position(33616, 32557, 13)},
    [2] = {fromPos = Position(33639, 32562, 13), toPos = Position(33617, 32557, 13)},
    [3] = {fromPos = Position(33640, 32562, 13), toPos = Position(33618, 32557, 13)},
    [4] = {fromPos = Position(33641, 32562, 13), toPos = Position(33619, 32557, 13)}
}

local monsters = {
    [1] = {pos = Position(33617, 32560, 13), name = "Faceless Bane"}
}
local quest_range = {fromPos = Position(33610, 32555, 13), toPos = Position(33626, 32569, 13)} -- see image in thread for explanation

local exit_position = Position(33640, 32560, 13) -- Position completely outside the quest area
...
local specs, spec = Game.getSpectators(Position(33919, 31646, 8), false, false, 18, 18, 18, 18)
    for j = 1, #specs do
        spec = specs[j]
        if spec:getName():lower() == 'Faceless Bane' then
            spec:remove()
        end
    end
end
...
function goshnarCruelty.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    for i = 1, #player_positions do
        ...
        if not tile then
            print(">> ERROR: Faceless Bane tile does not exist for Position(" .. fromPos.x .. ", " .. fromPos.y .. ", " .. fromPos.z .. ").")
    ...
    for i = 1, #monsters do
        ...
        if not Tile(toPos) then
            print(">> ERROR: Faceless Bane tile does not exist for Position(" .. toPos.x .. ", " .. toPos.y .. ", " .. toPos.z .. ").")
            return player:sendCancelMessage("There is an issue with this quest. Please contact an administrator.")
        end
        removeBoss()
