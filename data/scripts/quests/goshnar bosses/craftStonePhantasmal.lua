...
function craftStonePhantasmal.onUse(player, item, fromPosition, itemEx, toPosition)
    ...
    for k, v in pairs(craft.items) do
        if player:getItemCount(v.item) < v.count then
            player:sendTextMessage(MESSAGE_STATUS_SMALL, "You do not have necessary items, items necessary x4 Spectral Horse shoe, x1 Spectral Saddle, x1 Spectral Horse Tack.")
            player:getPosition():sendMagicEffect(CONST_ME_POFF)
            return true
        end
    end
